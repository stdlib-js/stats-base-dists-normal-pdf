...
tape( 'the function evaluates the pdf for `x` given parameters `mu` and `sigma`', opts, function test( t ) {
	var expected;
	var sigma;
	var mu;
	var x;
	...
	for ( i = 0; i < x.length; i++ ) {
		...
		t.strictEqual( isAlmostSameValue( y, expected[ i ], 250 ), true, 'returns expected value' );
	}
	t.end();
});
